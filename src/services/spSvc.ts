...
export class SpSvc<T> {
  ...
  public async Init() {
    const info = await sp.web.lists.getByTitle(this._listTitle).fields.select("Title", "StaticName", "Description", "TypeAsString", "TermSetId", "EntityPropertyName", "AllowMultipleValues", "LookupList", "LookupField", "Choices", "NumberOfLines", "RichText").filter('FromBaseType ne true').get<ISpFieldInfo[]>();
